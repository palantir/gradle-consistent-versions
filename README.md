...
By default, this function resolves a per-project view of the unified dependency graph to supply a version (see [ADR 1](docs/adr/0001-getversion-per-project-resolution.md) for how this works). You can also supply a different configuration to resolve a version from instead. Note that any configuration you supply must live in the project that `getVersion` is being called from:
