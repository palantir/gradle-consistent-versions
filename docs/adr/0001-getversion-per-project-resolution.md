...
- **Read the `gcvLocks` constraints instead of resolving.** The above error only applies to *resolution* — reading a configuration's declared `DependencyConstraint`s is fine, so the lookup could read the strict constraints off the root `gcvLocks` platform instead. But `gcvLocks` is just the configuration equivalent of `versions.lock`, so this has the same problem: it only covers external modules and can't serve a lookup that resolves to another project in the same build.
- **Per-project resolution of the unified classpath (chosen).** Introduce a dependency-scope configuration on the root holding all the declared dependencies, extended by a consumable configuration that exposes them to other projects. Each project then resolves its own configuration that consumes that root consumable configuration, so the lookup runs in the calling project but resolves the same dependencies that the root's `unifiedClasspath` did before.
