...
class CheckUnusedConstraintIntegrationTest {
    ...
    @Test
    @AdditionallyRunWithGradle("9.3.0")
    void allProjectsClasspath_collects_all_configurations_from_all_projects(
            GradleInvoker gradle, RootProject rootProject, SubProject foo, SubProject bar) {
        rootProject.file("versions.props").overwrite("""
            com.google.guava:guava = 33.0.0-jre
            org.slf4j:slf4j-api = 2.0.9
        ...
        foo.buildGradle().append("""
            ...
            // Create a custom resolvable configuration with a dependency that isn't in any Java configuration
            configurations {
                customFoo {
                    canBeResolved = true
                    canBeConsumed = false
            ...
            dependencies {
                implementation 'com.google.guava:guava'
                customFoo 'ch.qos.logback:logback-classic'  // Only in customFoo, not in implementation
            }
            """);
        ...
        // Verify that allProjectsClasspath exists and contains dependencies from custom configurations
        rootProject.buildGradle().append("""
            tasks.register('verifyAllProjectsClasspath') {
                doLast {
                    def allProjectsClasspath = configurations.findByName('allProjectsClasspath')
                    assert allProjectsClasspath != null : "allProjectsClasspath configuration should exist"

                    // The configuration should be resolvable
                    def resolved = allProjectsClasspath.resolvedConfiguration
                    def deps = resolved.resolvedArtifacts.collect { it.moduleVersion.id.toString() }

                    println "Resolved dependencies: ${deps}"

                    // Should contain guava (from foo's implementation)
                    assert deps.any { it.contains('guava') } : "Should contain guava from foo's implementation"
                    // Should contain slf4j-api (from bar's implementation)
                    assert deps.any { it.contains('slf4j-api') } : "Should contain slf4j-api from bar's implementation"
                    // Should contain logback-classic (from foo's customFoo - this proves custom configs are captured!)
                    assert deps.any { it.contains('logback-classic') } : "Should contain logback-classic from foo's custom configuration"
                }
            }
            """);

        InvocationResult result = gradle.withArgs("verifyAllProjectsClasspath").buildsSuccessfully();
        assertThat(result).task(":verifyAllProjectsClasspath").succeeded();
    }
