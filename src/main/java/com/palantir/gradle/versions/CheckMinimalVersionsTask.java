...
public class CheckMinimalVersionsTask extends DefaultTask {

    @InputFile
    public final RegularFileProperty propsFile = getProject().getObjects().fileProperty();
    private final Property<Boolean> shouldFix = getProject().getObjects().property(Boolean.class);

    public CheckMinimalVersionsTask() {
        shouldFix.set(false);
    }
    ...
    public final void check() throws IOException {
        RawVersionsProps parsedVersionsProps = RawVersionsProps.fromFile(propsFile.get().getAsFile());
        ...
        if (shouldFix.get()) {
            getProject().getLogger().lifecycle("Removing unused pins from versions.props:\n"
                    + linesToDelete);
            parsedVersionsProps.writebackSubsetOfLines(downgrades, propsFile.get().getAsFile());
            return;
        }
